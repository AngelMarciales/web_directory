<template>
    ...
    >
      <li>
        <a
          ...
          role="menuitem"
          tabindex="0"
          onclick="reset_password_modal_owner.showModal()"
          >Cambiar contraseña</a
        >
      </li>
      <li v-for="business in businessList" :key="business.id">
        <a
          class="dropdown-item"
          role="menuitem"
          tabindex="0"
  ...
  <dialog id="reset_password_modal_owner" class="modal">
    <div class="modal-box">
...
export default {
  ...
  mounted() {
    ...
    this.useUserStore.loadUserData();
  ...
  methods: {
    ...
    async getBusinessById() {
      ...
      try {
        await this.useBusinessStore.getByUserId(userid);
        this.businessList = this.useBusinessStore.businessesByUserId;
      } catch (error) {
        console.error(error);
      }
    },
    ...
    goToBusiness(businessOwner) {
      this.$router.push({
        name: "OwnerBusiness",
        params: { businessOwner: businessOwner },
      });
    },
  },
  beforeDestroy() {
